...
- **Never mix `import` and `export { X } from "..."` statements together** - group all `import` statements first, then a blank line, then all `export { X } from "..."` (re-export) statements
...
- Images: `src/app/assets/`
  - Sprites: `src/app/assets/sprites/`
  - Icons: `src/app/assets/icons/`
  - Media: `src/app/assets/media/`
- Fonts: `src/app/assets/fonts/`
- Reference assets using `assets/` path (not `src/app/assets/`) — the public URL is unaffected by the physical location
