...
describe("Damage Calculator Service", () => {
  ...
  beforeEach(() => {
    ...
    service = new DamageCalculatorService()
    service.adjusters = [adjusterOneSpy, adjusterTwoSpy] as unknown as CalcAdjuster[]
  })
  ...
  it("should adjust inputs before calculation", () => {
    ...
    expect(adjusterOneSpy.adjust).toHaveBeenCalledWith(expect.any(CalcPokemon), expect.any(CalcPokemon), activeMove, expect.any(MoveCalc), expect.any(FieldCalc), undefined, expect.any(Field))
    expect(adjusterTwoSpy.adjust).toHaveBeenCalledWith(expect.any(CalcPokemon), expect.any(CalcPokemon), activeMove, expect.any(MoveCalc), expect.any(FieldCalc), undefined, expect.any(Field))
  })

  it("should inject adjusted BP in description when adjuster sets override", () => {
    adjusterOneSpy.adjust.mockImplementation((_a, _t, _m, moveCalc) => {
      moveCalc.bp = 150
  ...
  it("should inject adjusted BP in description for both attackers when both have overrides", () => {
    adjusterOneSpy.adjust.mockImplementation((_a, _t, move, moveCalc) => {
  ...
  it("should adjust inputs before calculation when have second attacker", () => {
    ...
    expect(adjusterOneSpy.adjust).toHaveBeenCalledWith(expect.any(CalcPokemon), expect.any(CalcPokemon), activeMove, expect.any(MoveCalc), expect.any(FieldCalc), attacker, expect.any(Field))
    expect(adjusterTwoSpy.adjust).toHaveBeenCalledWith(expect.any(CalcPokemon), expect.any(CalcPokemon), activeMove, expect.any(MoveCalc), expect.any(FieldCalc), attacker, expect.any(Field))
  })
