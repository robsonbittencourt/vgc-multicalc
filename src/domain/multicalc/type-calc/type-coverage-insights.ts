import { TypeCoverage, OffensiveCoverageData, DefensiveCoverageData, OffensiveCoverageAgainstTeamData, DefensiveCoverageByPokemonData } from "./type-coverage"
...
export class TypeCoverageInsights {
  private typeCoverage = new TypeCoverage()
  private typeChart = new TypeChart()
  ...
  selectOffensiveSuperEffective(team: Team, secondTeam: Team | null): PokemonInsight[] {
    return this.getTopOffensiveSuperEffective(team, secondTeam).slice(0, 2)
  }

  selectOffensiveNotVeryEffective(team: Team, secondTeam: Team | null): PokemonInsight[] {
    const superEffectiveIds = new Set(this.selectOffensiveSuperEffective(team, secondTeam).map(item => item.pokemon.id))

    return this.getTopOffensiveNotVeryEffective(team, secondTeam)
      .filter(item => !superEffectiveIds.has(item.pokemon.id))
      .slice(0, 2)
  }

  selectOffensivePokemon(team: Team, secondTeam: Team | null): PokemonInsight[] {
    const superEffective = this.getTopOffensiveSuperEffective(team, secondTeam)
    const notVeryEffective = this.getTopOffensiveNotVeryEffective(team, secondTeam)
  ...
  selectDefensivePositive(team: Team, secondTeam: Team | null): PokemonInsight[] {
    return this.getTopDefensivePositive(team, secondTeam).slice(0, 2)
  }

  selectDefensiveWeak(team: Team, secondTeam: Team | null): PokemonInsight[] {
    return this.getTopDefensiveWeak(team, secondTeam).slice(0, 2)
  }

  selectDefensivePokemon(team: Team, secondTeam: Team | null): PokemonInsight[] {
    const positive = this.selectDefensivePositive(team, secondTeam)
    const positiveIds = new Set(positive.map(p => p.pokemon.id))

    const weak = this.getTopDefensiveWeak(team, secondTeam)
      .filter(item => !positiveIds.has(item.pokemon.id))
      .slice(0, 2)
  ...
  getPokemonWeaknessesCoveredByTera(pokemon: Pokemon): number {
    ...
    PokemonTypes.forEach(moveType => {
      const originalEffectiveness = this.typeChart.getEffectiveness(moveType, type1, type2)
      if (originalEffectiveness === 2 || originalEffectiveness === 4) {
        const teraEffectiveness = this.typeChart.getEffectiveness(moveType, teraType, undefined)
  ...
  private calculateOffensiveSuperEffectiveStats(team: Team, secondTeam: Team | null): { pokemon: Pokemon; count: number; count4x: number }[] {
    const validTeamMembers = team.teamMembers
  ...
  private getOffensiveCoverageData(team: Team, secondTeam: Team | null): OffensiveCoverageData[] | OffensiveCoverageAgainstTeamData[] {
    if (secondTeam) {
      return this.typeCoverage.getOffensiveCoverageAgainstTeam(team, secondTeam, false, false)
    }

    return this.typeCoverage.getOffensiveCoverage(team)
  }

  private getDefensiveCoverageData(team: Team, secondTeam: Team | null): DefensiveCoverageData[] | DefensiveCoverageByPokemonData[] {
    if (secondTeam) {
      return this.typeCoverage.getDefensiveCoverageAgainstTeam(team, secondTeam, false, false)
    }

    return this.typeCoverage.getDefensiveCoverage(team, false)
  }
}
