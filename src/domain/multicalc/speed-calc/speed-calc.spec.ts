...
class StoreStub {
  private targetsState: Target[] = []
  private teamsState: Team[] = []

  updateTargets(targets: Target[]) {
    this.targetsState = targets
  }
  ...
  team(): Team {
    return this.teamsState.find(t => t.active) ?? new Team("", true, "", [])
  }
}

function teamPokemonFrom(store: StoreStub, teamId = ""): SpeedTeamPokemon {
  const opponents = store
    .targets()
    .flatMap(t => [t.pokemon, t.secondPokemon])
    .filter((p): p is Pokemon => p != null)

  const selectedTeam = store.teams().find(t => t.id === teamId)
  const team = selectedTeam ? selectedTeam.teamMembers.map(m => m.pokemon) : []

  const myTeam = store.team().teamMembers.map(m => m.pokemon)

  return { opponents, team, myTeam }
}

describe("SpeedCalc", () => {
  let service: SpeedCalc
  let store: StoreStub
  ...
  describe("Test order methods", () => {
    ...
    it("should not apply opponent side modifiers to my team pokemon", () => {
      ...
      const withModifier = new SpeedCalcOptions({ filterType: "opponents", showMyTeam: true, speedModifier: -1 })

      const baseline = service.orderedPokemon(pokemon, field, pokemonEachSide, teamPokemonFrom(store), withoutModifier).find(s => s.description.includes(YOUR_TEAM) && s.pokemonName === "Rillaboom")!
      const modified = service.orderedPokemon(pokemon, field, pokemonEachSide, teamPokemonFrom(store), withModifier).find(s => s.description.includes(YOUR_TEAM) && s.pokemonName === "Rillaboom")!
    ...
    it("should apply attacker side field effects to my team pokemon", () => {
      ...
      const noTailwind = service.orderedPokemon(pokemon, new Field(), pokemonEachSide, teamPokemonFrom(store), options).find(s => s.description.includes(YOUR_TEAM) && s.pokemonName === "Rillaboom")!
      const tailwindField = new Field({ attackerSide: new FieldSide({ isTailwind: true }) })
      const withTailwind = service.orderedPokemon(pokemon, tailwindField, pokemonEachSide, teamPokemonFrom(store), options).find(s => s.description.includes(YOUR_TEAM) && s.pokemonName === "Rillaboom")!
    ...
    it("should not mark Speed Tie when the same pokemon is the actual and in the team", () => {
      ...
      const field = new Field()
      const pokemonEachSide = 30
      const options = new SpeedCalcOptions({ filterType: "team", teamId: "team-1" })

      const inRange = service.orderedPokemon(flutterMane, field, pokemonEachSide, teamPokemonFrom(store, "team-1"), options)
    ...
    it("should not duplicate the team pokemon in Base mode", () => {
      ...
      const field = new Field()
      const pokemonEachSide = 30
      const options = new SpeedCalcOptions({ filterType: "team", teamId: "team-1", mode: SpeedCalcMode.Base })
    ...
    it("should not add empty padding for opponents below the no padding threshold", () => {
      ...
      const inRange = service.orderedPokemon(pokemon, field, pokemonEachSide, teamPokemonFrom(store), options, 19)

      expect(inRange.some(s => s.isPadding)).toBe(false)
    ...
    it("should add empty padding for opponents at or above the no padding threshold", () => {
      ...
      const inRange = service.orderedPokemon(pokemon, field, pokemonEachSide, teamPokemonFrom(store), options, 1)

      expect(inRange.some(s => s.isPadding)).toBe(true)
    })
    ...
    it("should return Pokémon in speed range ordered with default options", () => {
      ...
      const inRange = service.orderedPokemon(pokemon, field, pokemonEachSide, teamPokemonFrom(store))
    ...
    it("should fill left side with empty Speed Definitions of actual until have pokemonEachSide value", () => {
      ...
      expect(inRange[0].isPadding).toBe(true)
    })
