...
describe("Aura abilities", () => {
  const blissey = (options: Record<string, unknown> = {}) => new Pokemon("Blissey", { sps: { hp: 32 }, ...options } as never)
  ...
  it("boosts a Fairy move when Fairy Aura is active on the field", () => {
    ...
    expect(result.description()).toEqual("32+ SpA Sylveon Fairy Aura Moonblast vs. 32 HP / 0 SpD Blissey: 82-97 (22.6 - 26.7%) -- 31.1% chance to 4HKO")
  })

  it("boosts a Dark move when Dark Aura is active on the field", () => {
    const attacker = new Pokemon("Hydreigon", { sps: { spa: 32 }, nature: "Modest" })

    const result = calculate(attacker, blissey(), new Move("Dark Pulse"), new Field({ isDarkAura: true } as never))

    expect(result.description()).toEqual("32+ SpA Hydreigon Dark Pulse vs. 32 HP / 0 SpD Blissey: 76-90 (20.9 - 24.8%) -- guaranteed 5HKO")
  })
  ...
  it("weakens the aura boost when the defender has Aura Break", () => {
    ...
    expect(result.description()).toEqual("32+ SpA Sylveon Moonblast vs. 32 HP / 0 SpD Blissey: 46-55 (12.7 - 15.1%) -- possible 7HKO")
  })
})
...
describe("Attack-boosting abilities and items", () => {
  ...
  it("applies Sheer Force to Electro Shot, which has no secondary effect entry", () => {
    const attacker = new Pokemon("Raging Bolt", { sps: { spa: 32 }, ability: "Sheer Force" })

    const result = calculate(attacker, blissey(), new Move("Electro Shot"), new Field())

    expect(result.description()).toEqual("+1 32 SpA Sheer Force Raging Bolt Electro Shot vs. 32 HP / 0 SpD Blissey: 174-205 (48 - 56.6%) -- 87.5% chance to 2HKO")
  })

  it("applies Sheer Force to Order Up", () => {
    const attacker = new Pokemon("Dondozo", { sps: { atk: 32 }, ability: "Sheer Force" })

    const result = calculate(attacker, blissey(), new Move("Order Up"), new Field())

    expect(result.description()).toEqual("32 Atk Sheer Force Dondozo Order Up vs. 32 HP / 0 Def Blissey: 198-233 (54.6 - 64.3%) -- guaranteed 2HKO")
  })

  it("applies Reckless to a move with crash damage rather than recoil", () => {
    const attacker = new Pokemon("Hitmonlee", { sps: { atk: 32 }, ability: "Reckless" })

    const result = calculate(attacker, blissey(), new Move("High Jump Kick"), new Field())

    expect(result.description()).toEqual("32 Atk Reckless Hitmonlee High Jump Kick vs. 32 HP / 0 Def Blissey: 1004-1184 (277.3 - 327%) -- guaranteed OHKO")
  })

  it("applies Iron Fist to a punching move", () => {
    const attacker = new Pokemon("Ursaluna", { sps: { atk: 32 }, ability: "Iron Fist" })

    const result = calculate(attacker, blissey(), new Move("Drain Punch"), new Field())

    expect(result.description()).toEqual("32 Atk Iron Fist Ursaluna Drain Punch vs. 32 HP / 0 Def Blissey: 432-510 (119.3 - 140.8%) -- guaranteed OHKO")
  })

  it("stacks the Metronome item boost with the number of consecutive uses", () => {
    const attacker = new Pokemon("Garchomp", { sps: { atk: 32 }, item: "Metronome" })

    const result = calculate(attacker, blissey(), new Move("Earthquake", { timesUsedWithMetronome: 3 } as never), new Field())

    expect(result.description()).toEqual("32 Atk Metronome Garchomp Earthquake vs. 32 HP / 0 Def Blissey: 544-643 (150.2 - 177.6%) -- guaranteed OHKO")
  })
  ...
  it("does not apply the Metronome boost when the move has no usage count", () => {
    ...
    expect(result.description()).toEqual("32 Atk Garchomp Earthquake vs. 32 HP / 0 Def Blissey: 340-402 (93.9 - 111%) -- 62.5% chance to OHKO")
  })
...
describe("Defensive Quark Drive and Multiscale", () => {
  const ironTreads = (boostedStat: string) => new Pokemon("Iron Treads", { sps: { hp: 32 }, ability: "Quark Drive", boostedStat } as never)
  ...
  it("boosts Def against a physical move when Quark Drive picked Def", () => {
    ...
    expect(result.description()).toEqual("32 Atk Garchomp Earthquake vs. 32 HP / 0 Def Quark Drive Iron Treads: 116-138 (58.8 - 70%) -- guaranteed 2HKO")
  })

  it("boosts SpD against a special move when Quark Drive picked SpD", () => {
    const attacker = new Pokemon("Hydreigon", { sps: { spa: 32 } })

    const result = calculate(attacker, ironTreads("spd"), new Move("Dark Pulse"), electricTerrain())

    expect(result.description()).toEqual("32 SpA Hydreigon Dark Pulse vs. 32 HP / 0 SpD Quark Drive Iron Treads: 69-82 (35 - 41.6%) -- guaranteed 3HKO")
  })
  ...
  it("does not boost SpD against a special move when Quark Drive picked Def", () => {
    ...
    expect(result.description()).toEqual("32 SpA Hydreigon Dark Pulse vs. 32 HP / 0 SpD Iron Treads: 90-106 (45.6 - 53.8%) -- 41.4% chance to 2HKO")
  })

  it("halves the damage taken by a Shadow Shield holder at full HP", () => {
    const attacker = new Pokemon("Garchomp", { sps: { atk: 32 } })
    const defender = new Pokemon("Lunala", { sps: { hp: 32 }, ability: "Shadow Shield" })

    const result = calculate(attacker, defender, new Move("Earthquake"), new Field())

    expect(result.description()).toEqual("32 Atk Garchomp Earthquake vs. 32 HP / 0 Def Shadow Shield Lunala: 47-56 (19.2 - 22.9%) -- 96.7% chance to 3HKO")
  })
})

describe("Aura abilities on the defending side", () => {
  const sylveon = (options: Record<string, unknown> = {}) => new Pokemon("Sylveon", { sps: { spa: 32 }, nature: "Modest", ...options } as never)
  const xerneas = () => new Pokemon("Xerneas", { sps: { hp: 32 }, ability: "Fairy Aura" })
  ...
  it("boosts the move when the defender is the one holding Fairy Aura", () => {
    ...
    expect(result.description()).toEqual("32+ SpA Sylveon Moonblast vs. 32 HP / 0 SpD Fairy Aura Xerneas: 108-127 (46.3 - 54.5%) -- 55.9% chance to 2HKO")
  })
  ...
  it("weakens the defender's aura when the attacker has Aura Break", () => {
    ...
    expect(result.description()).toEqual("32+ SpA Sylveon Moonblast vs. 32 HP / 0 SpD Fairy Aura Xerneas: 61-73 (26.1 - 31.3%) -- guaranteed 4HKO")
  })
})

describe("Pinch and teamwork abilities", () => {
  const blissey = () => new Pokemon("Blissey", { sps: { hp: 32 } })
  const inPinch = (name: string, ability: string) => new Pokemon(name, { sps: { spa: 32 }, ability, curHP: 1 } as never)
  ...
  it("boosts a Grass move with Overgrow at low HP", () => {
    ...
    expect(result.description()).toEqual("32 SpA Overgrow Venusaur Giga Drain vs. 32 HP / 0 SpD Blissey: 63-75 (17.4 - 20.7%) -- possible 5HKO")
  })
  ...
  it("boosts a Fire move with Blaze at low HP", () => {
    ...
    expect(result.description()).toEqual("32 SpA Blaze Charizard Flamethrower vs. 32 HP / 0 SpD Blissey: 79-94 (21.8 - 25.9%) -- 3.8% chance to 4HKO")
  })
  ...
  it("boosts a Water move with Torrent at low HP", () => {
    ...
    expect(result.description()).toEqual("32 SpA Torrent Blastoise Surf vs. 32 HP / 0 SpD Blissey: 67-81 (18.5 - 22.3%) -- possible 5HKO")
  })
  ...
  it("boosts a Bug move with Swarm at low HP", () => {
    ...
    expect(result.description()).toEqual("32 SpA Swarm Volcarona Bug Buzz vs. 32 HP / 0 SpD Blissey: 93-109 (25.6 - 30.1%) -- guaranteed 4HKO")
  })

  it("boosts a special move when Plus is active", () => {
    const attacker = new Pokemon("Plusle", { sps: { spa: 32 }, ability: "Plus", abilityOn: true } as never)

    const result = calculate(attacker, blissey(), new Move("Thunderbolt"), new Field())

    expect(result.description()).toEqual("32 SpA Plus Plusle Thunderbolt vs. 32 HP / 0 SpD Blissey: 67-81 (18.5 - 22.3%) -- possible 5HKO")
  })

  it("boosts a special move when Minus is active", () => {
    const attacker = new Pokemon("Minun", { sps: { spa: 32 }, ability: "Minus", abilityOn: true } as never)

    const result = calculate(attacker, blissey(), new Move("Thunderbolt"), new Field())

    expect(result.description()).toEqual("32 SpA Minus Minun Thunderbolt vs. 32 HP / 0 SpD Blissey: 63-75 (17.4 - 20.7%) -- possible 5HKO")
  })
})

describe("Multiscale and entry hazards", () => {
  const garchomp = () => new Pokemon("Garchomp", { sps: { atk: 32 } })
  const dragonite = (options: Record<string, unknown> = {}) => new Pokemon("Dragonite", { sps: { hp: 32 }, ability: "Multiscale", ...options } as never)
  ...
  it("does not apply Multiscale when Stealth Rock has chipped the defender", () => {
    ...
    expect(result.description()).toEqual("0 SpA Garchomp Ice Beam vs. 32 HP / 0 SpD Dragonite: 116-140 (58.5 - 70.7%) -- guaranteed 2HKO after Stealth Rock")
  })
  ...
  it("applies Multiscale against Spikes when the defender is Flying and stays ungrounded", () => {
    ...
    expect(result.description()).toEqual("0 SpA Garchomp Ice Beam vs. 32 HP / 0 SpD Multiscale Dragonite: 58-70 (29.2 - 35.3%) -- 23.8% chance to 2HKO")
  })
  ...
  it("does not apply Shadow Shield when Spikes chip a grounded defender", () => {
    ...
    expect(result.description()).toEqual("0 SpA Garchomp Shadow Ball vs. 32 HP / 0 SpD Lunala: 96-116 (39.3 - 47.5%) -- 84% chance to 2HKO after 2 layers of Spikes")
  })
...
describe("Knock Off against an unremovable item", () => {
  ...
  it("does not boost Knock Off when the defender holds its own mega stone", () => {
    const defender = new Pokemon("Garchomp", { sps: { hp: 32 }, item: "Garchompite" } as never)

    const result = calculate(garchomp(), defender, new Move("Knock Off"), new Field())

    expect(result.description()).toEqual("32 Atk Garchomp Knock Off vs. 32 HP / 0 Def Garchomp: 39-47 (18.1 - 21.8%) -- possible 5HKO")
  })

  it("does not boost Knock Off when the already-mega defender holds the stone that created it", () => {
    const defender = new Pokemon("Garchomp-Mega", { sps: { hp: 32 }, item: "Garchompite" } as never)

    const result = calculate(garchomp(), defender, new Move("Knock Off"), new Field())

    expect(result.description()).toEqual("32 Atk Garchomp Knock Off vs. 32 HP / 0 Def Garchomp-Mega: 34-40 (15.8 - 18.6%) -- possible 6HKO")
  })

  it("boosts Knock Off when the defender holds a removable item", () => {
    const defender = new Pokemon("Garchomp", { sps: { hp: 32 }, item: "Leftovers" } as never)
