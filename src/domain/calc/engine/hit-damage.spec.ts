...
describe("computeAttack — Power Trick on the attacker's side", () => {
  ...
  const garchomp = () => new Pokemon("Garchomp", { sps: { atk: 32, spa: 32, def: 32 }, nature: "Adamant" })
  ...
  it("swaps Atk and Def for a regular physical move", () => {
    ...
    expect(result.description()).toEqual("32 Atk (Def) Garchomp with Power Trick Earthquake vs. 32 HP / 0 Def Blissey: 276-325 (76.2 - 89.7%) -- guaranteed 2HKO")
  })
  ...
  it("swaps Atk and Def for Body Press, which attacks with Def", () => {
    ...
    expect(result.description()).toEqual("32+ Def (Atk) Garchomp with Power Trick Body Press vs. 32 HP / 0 Def Blissey: 400-472 (110.4 - 130.3%) -- guaranteed OHKO")
  })
  ...
  it("leaves a special move untouched, since Power Trick only swaps Atk and Def", () => {
    ...
    expect(result.description()).toEqual("32- SpA Garchomp Flamethrower vs. 32 HP / 0 SpD Blissey: 27-32 (7.4 - 8.8%)")
  })
  ...
  it("leaves Foul Play untouched, since it uses the defender's Atk instead", () => {
    ...
    expect(result.description()).toEqual("0 Atk Garchomp Foul Play vs. 32 HP / 0 Def Blissey: 36-43 (9.9 - 11.8%) -- possible 9HKO")
  })
})
