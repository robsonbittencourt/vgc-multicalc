...
export class Move {
  ...
  constructor(name: string, options: { alliesFainted?: string; hits?: string } = {}) {
    ...
    const moveName = name?.toLowerCase().replaceAll(" ", "").replaceAll("-", "")
    if (moveName) {
      const moveDetails = MOVE_DETAILS[moveName]
      this.accuracy = moveDetails.accuracy === true ? 100 : typeof moveDetails.accuracy === "number" ? moveDetails.accuracy : 100
