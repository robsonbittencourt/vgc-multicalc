...
import { Species } from "@robsonbittencourt/calc/dist/data/species"
...
export class AllPokemon {
  ...
  private constructor() {
    this.allPokemon = Object.entries(FormatsData)
      .filter(([key, value]) => value.tier != "Illegal" && value.isNonstandard != "CAP" && value.isNonstandard != "Past")
      .map(([key]) => {
        return Pokedex[key]
      })
...
namespace TierTypes {
  export type Singles = "AG" | "Uber" | "(Uber)" | "OU" | "(OU)" | "UUBL" | "UU" | "RUBL" | "RU" | "NUBL" | "NU" | "(NU)" | "PUBL" | "PU" | "(PU)" | "ZUBL" | "ZU" | "NFE" | "LC"
...
const FormatsData: { [k: string]: SpeciesFormatsData } = {
  bulbasaur: {
    tier: "LC"
  },
  ivysaur: {
